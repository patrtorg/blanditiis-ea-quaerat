# @patrtorg/blanditiis-ea-quaerat

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@patrtorg/blanditiis-ea-quaerat'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@patrtorg/blanditiis-ea-quaerat` preserves information of big numbers. `@patrtorg/blanditiis-ea-quaerat` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@patrtorg/blanditiis-ea-quaerat` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@patrtorg/blanditiis-ea-quaerat):

```
npm install @patrtorg/blanditiis-ea-quaerat
```
...
```js
import { parse } from '@patrtorg/blanditiis-ea-quaerat'
...
```js
import { parse, isInteger } from '@patrtorg/blanditiis-ea-quaerat'
...
```js
import { parse, isSafeNumber } from '@patrtorg/blanditiis-ea-quaerat'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@patrtorg/blanditiis-ea-quaerat'
...
(Spoiler: `@patrtorg/blanditiis-ea-quaerat` is much slower than native)
